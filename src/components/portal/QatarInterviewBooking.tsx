...
export function QatarInterviewBooking({
  ...
  const initiatePayment = async () => {
    ...
    try {
      ...
      try {
        const rawUser = localStorage.getItem('qatarJobsUser');
        if (rawUser) {
              ...
              {(step === "details" || step === "schedule") && (
                <div className="p-4 rounded-xl bg-gradient-to-r from-primary/5 to-secondary/5 border border-border/60">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-foreground">You’re almo24t there</p>
                  ...
                  {(selectedDate || selectedTime) && (
                    <div className="p-4 rounded-xl border border-border/60 bg-card">
                      ...
                      <p className="text-xs text-muted-foreground mt-2">
                        Next: confirm with a one-time verification payment of 240 to lock this slot.
                      </p>
                    </div>
                  )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-xl border border-border/60 bg-card">
                    <p className="font-semibold text-foreground flex items-center gap-2">
                      <CreditCard className="w-4 h-4 text-secondary" /> Verification Payment: 240
                    </p>
                    <div className="mt-3 flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-between">
